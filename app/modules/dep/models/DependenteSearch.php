...
/**
 * DependentesSearch represents the model behind the search form of `app\modules\dep\models\Dependente`.
 */
class DependenteSearch extends Dependente
{

    /**
     * {@inheritdoc}
     */
    public $responsavel, $parentesco;
    ...
    public function rules()
    {
        return [
            ...
            [[
                'nm_ide_pes', 'nm_nom_dep', 'dt_nas_dep', 'nu_num_cpf', 'nu_num_ide', 'nm_nom_obs',
                'nm_des_cid', 'nu_cod_cid', 'dt_tim_cri', 'dt_tim_mod', 'responsavel', 'parentesco'
            ], 'safe'],
            [['nu_ren_dep'], 'number'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere(['dependente.bo_reg_exc' => 0]);

        //$query = Dependente::find(['bo_reg_exc' => 0]);
        ...
        /* if (Yii::$app->controller->action->id != 'dependente') {
          $dataProvider->setSort(['defaultOrder' => ['responsavel' => SORT_ASC]]);
          } else {
          $dataProvider->setSort(['defaultOrder' => ['nm_nom_dep' => SORT_ASC]]);
          };*/

        $dataProvider->sort->attributes['parentesco'] = [
            // The tables are the ones our relation are configured to
            // in my case they are prefixed with "tbl_"
            'asc' => ['ger_parentesco.nm_nom_par' => SORT_ASC],
            'desc' => ['ger_parentesco.nm_nom_par' => SORT_DESC],
        ];
        /* $dataProvider->sort->attributes['responsavel'] = [
          ...
          //'defaulOrder' => SORT_ASC,
          ];  */

        $query->joinWith(['numRes', 'numPar', 'numNat', 'graIns', 'estCiv', 'numGen', 'oriRen'], 'RIGHT JOIN');
        $this->load($params);
        ...
        //->andFilterWhere(['like', 'responsavel.nm_nom_res', $this->responsavel])
        // ->andFilterWhere(['like', 'ger_parentesco.nm_nom_par', $this->parentesco])
