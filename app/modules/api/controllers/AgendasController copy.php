...
class AgendasController extends Controller
{
    ...
    public function actionGetAllSchedule()
    {
        ...
        /*  $modelAsub = Agenda::find()
            ->select(['dt_age_dat', 'COUNT(*) AS nu_tot_dia'])
            ->where(['=', 'bo_reg_exc', 0])
        ...
        /*   function myFunction($value, $key)
        {
         ...
         array_walk_recursive($ti_in_day, "myFunction");
 */
        $dt_in_mon = array_keys($ti_in_day);
        if (count($modelA) > 0) {
            return [
                'name' => 'Localizada',
    ...
    {
        $modelA = new Agenda();
        $t = Yii::$app->getRequest();
        $sheduleUser = new DateTime(Yii::$app->getRequest()->getBodyParam('dt_age_dat'));
        $nuCpf = Yii::$app->getRequest()->getBodyParam('nu_num_cpf');
        $datelibsub = new DateTime(Yii::$app->getRequest()->getBodyParam('dt_age_dat'));
        $datelibadd = new DateTime(Yii::$app->getRequest()->getBodyParam('dt_age_dat'));
        $datelibsub->sub(new DateInterval('P30D'));
        $datelibadd->add(new DateInterval('P30D'));
    ...
    public function actionGetCounterMonth()
    {
        ...
        foreach ($dates as $value) {
            ...
            //$datesValidate = array_merge($holidays, $disabledays, $disabledates);
    ...
    function actionGetAllFullDate()
    {
        ...
        foreach ($countDate as $key => $value) {
            if (strtotime($key) < strtotime($dateQtd[0])) {
                if (($value >= 8) && (in_array(date('w', strtotime($key)), [1, 5]))) {
                    array_push($dateFull, ['dt_age_dat' => date('d-m-Y', strtotime($key))]);
                }
                if (($value >= 12) && (in_array(date('w', strtotime($key)), [2, 3, 4]))) {
                    array_push($dateFull, ['dt_age_dat' => date('d-m-Y', strtotime($key))]);
                }
            } else {
    ...
    public function actionGetUserSchedule()
    {
        ...
        if (count($modelA) > 0) {
            ...
            return [
                'name' => 'Não localizado',
                'message' => 'Histórico não localizada.',
                'success' => false,
                'dtserver' => date("Y-m-d H:i:s"),
                'status' => 'Erro',
            ];
