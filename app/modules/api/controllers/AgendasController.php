...
class AgendasController extends Controller
{
   ...
   /*  public function behaviors()
    ...
    public function actionGetAllSchedule()
    {
        ...
        $in_hol =
            ArrayHelper::getColumn(Yii::$app->session->get('all_date_holiday'), 'dt_age_fer');
    ...
    public function actionGetCounterMonth()
    {
        ...
        $dt = [];
        $dateYear = date('Y');
        $enbMonth = Yii::$app->session->get('all_month');
        $dates = ArrayHelper::getColumn($enbMonth, 'nu_num_mes');

        $datesDisableFer = Yii::$app->session->get('all_date_holiday');
        $datesDisableDay = Yii::$app->session->get('all_date_disable');

        $datesDisableDayFul = ['results' => ['dt_age_dat' => '']];
        $response = $client->createRequest()
            ->setMethod('GET')
            ->setUrl([
            ...
            ->send();
        if ($response->isOk) {
            $datesDisableDayFul = $response->data['data'];
        }

        $holidays = ArrayHelper::getColumn($datesDisableFer, 'dt_age_fer');
        $disabledays = ArrayHelper::getColumn($datesDisableDay, 'dt_age_dat');
        $disabledaysfull = ArrayHelper::getColumn($datesDisableDayFul, 'dt_age_dat');

        foreach ($dates as $value) {
            // for ($i = 1; $i <= 12; $i++) {
            $dateNow = date("Y-m-d");
            $date = date("Y-m-d", strtotime('01-' . $value . '-' . $dateYear));
            //$dateMonth = date('n', strtotime($date));

            $datesFullMonth = ArrayHelper::getColumn($datesFullMonth = MarArtHelpers::getFullDays($dateYear, $value, '01'), 'dt_age_dat');
            ...
            $datesValidate = array_diff(
                ...
                empty(implode($disabledaysfull)) ? ['01-01-2000'] : $disabledaysfull
            );
