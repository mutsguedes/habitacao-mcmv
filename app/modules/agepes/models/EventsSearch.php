<?php

namespace app\modules\agepes\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\modules\agepes\models\Events;

/**
 * EventsSearch represents the model behind the search form of `app\modules\agepes\models\Events`.
 */
class EventsSearch extends Events
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'id_user'], 'integer'],
            [['title', 'text', 'start', 'end', 'all_day', 'color_background', 'color_text', 'status'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Events::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'id_user' => $this->id_user,
            'start' => $this->start,
            'end' => $this->end,
        ]);

        $query->andFilterWhere(['like', 'title', $this->title])
            ->andFilterWhere(['like', 'text', $this->text])
            ->andFilterWhere(['like', 'all_day', $this->all_day])
            ->andFilterWhere(['like', 'color_background', $this->color_background])
            ->andFilterWhere(['like', 'color_text', $this->color_text])
            ->andFilterWhere(['like', 'status', $this->status]);

        return $dataProvider;
    }
}
