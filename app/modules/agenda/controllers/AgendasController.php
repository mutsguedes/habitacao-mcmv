...
class AgendasController extends Controller
{
    ...
    public function actionPreCreate()
    {
        ...
        $modelA->id_num_sta = 1;
        $datesValidate = [];
        $datesDisableFer = ArrayHelper::getColumn(Yii::$app->session->get('all_date_holiday'), 'dt_age_fer');
        $datesDisableDay = ArrayHelper::getColumn(Yii::$app->session->get('all_date_disable'), 'dt_age_dat');
        $datesDisableDayFull = ArrayHelper::getColumn(Yii::$app->runAction('/agenda/agendas/get-full-date'), 'dt_age_dat');

        $datesValidate = array_merge($datesDisableFer, $datesDisableDay, $datesDisableDayFull);
    ...
    function actionGetFullDate()
    {
        ...
        try {
            ...
            die('my exception handling: ' .  $e->getMessage() . "\n");
        }
        return $out;
    }
    ...
    public function actionGetValidateDates($dateYear, $dateMonth)
    {
        ...
        $datesValidate = array_diff(
            ...
            empty(implode($datesDisableDayFull)) ? ['01-01-2000'] : $datesDisableDayFull
        );
    ...
    {
        $dt = [];
        $enbMonth = ArrayHelper::getColumn(Yii::$app->session->get('all_months'), 'nu_num_mes');
        for ($i = 1; $i <= 12; $i++) {
            $dateNow = date("Y-m-d");
